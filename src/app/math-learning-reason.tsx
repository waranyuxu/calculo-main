...
export default function MathLearningReason() {
  ...
  const handleContinue = async () => {
    ...
    try {
      await saveOnboardingPreferences({ learningReason: selected }, userId);
      router.push("/discovery-source");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <ProgressBar
        ...
        onBack={() => router.back()}
      />
      <MascotHeader variant="smile">
        <Text style={[styles.bubbleText, { color: colors.text }]}>
          {copy.onboarding.learningReasonQuestion}
        </Text>
      </MascotHeader>
      <ScrollView
        style={{ flex: 1 }}
      ...
      >
        {copy.onboarding.learningReasons.map((label) => (
          <OptionRow
            key={label}
            label={label}
