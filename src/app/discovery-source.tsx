...
export default function DiscoverySource() {
  ...
  const handleContinue = async () => {
    ...
    try {
      await saveOnboardingPreferences({ discoverySource: selected }, userId);
      router.push("/math-study-routine");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      ...
      <MascotHeader variant="study">
        <Text style={[styles.bubbleText, { color: colors.text }]}>
          {copy.onboarding.discoveryQuestion}
        </Text>
      </MascotHeader>
      ...
      >
        {copy.onboarding.discoverySources.map((s) => (
          <OptionRow
            key={s.label}
            icon={s.icon}
            label={s.label}
